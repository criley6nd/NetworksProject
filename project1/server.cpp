...
main(int argc, char* argv[])
{
  ...
  while (!isEnd) {
    memset(buf, '\0', sizeof(buf)); //buffer size should be 1024, that is considered the magic number for sending over the network

    if (recv(clientSockfd, buf, buffsize, 0) == -1) { //only worry about first three arguments
      perror("recv");
      return 5;
    }
